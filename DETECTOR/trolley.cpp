...
const double TROLLEY::_absDiscrepancyMaxOfCoord = 3.0; // максимальна€ нев€зка координаты
...
// coord, coordL, coordR, time  задают требование оказатьс€ в координате в заданный момент времени
//
...
// расчет скорости мм/мс, котора€ требуетс€, чтобы переместитьс€ из currentCoordinate в targetCoordinate за timePeriod
// координаты задабютс€ в мм, врем€ - в мс
...
//                    qDebug() << "Stopped without delay on coordinate" << _coordinate << "_coordL =" << _coordinate + _rotationDegree * 0.5 << "_coordR =" << _coordinate - _rotationDegree * 0.5;
               _movingState = Normal;
                   ...
                   {// определение скорости торможени€, чтобы остановитьс€ за 2.0 м—ы
           ...
           else
           {
                ...
                if (fabs(coordDiscrepancy) > _absDiscrepancyMaxOfCoord)
                {
                    ...
                    if (_targetV >= 0)
                    {
                        ...
                        { // требуетс€ резкое снижение скорости
